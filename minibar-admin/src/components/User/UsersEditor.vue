      ...
      <el-col :span="22" :offset="1">
        <el-form :model="userForm" :rules="rules" ref="userForm" label-width="100px" class="demo-userForm">
          <el-form-item label="用户名" prop="username">
            <span v-if="isEditor">{{userForm.username}}</span>
            <el-input v-else v-model="userForm.username"></el-input>
          </el-form-item>
          ...
          <el-form-item label="密码" prop="password">
            <span v-if="isEditor">{{userForm.password}}</span>
            <el-input v-else v-model="userForm.password" type="password"></el-input>
          </el-form-item>
...
import { postCreate, getById, postUpdate } from "./../../serveice/member";
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.isEditor) {
            ...
            // 新增
            postCreate(this.userForm).then(res => {
              if (res.data.status == "y") {
                // 成功之后处理
                this.$message({
                  message: res.data.msg,
